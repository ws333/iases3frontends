// indexedDB.ts generated by Grok 3 beta
...
export async function getActiveContacts(): Promise<ContactI3C[]> {
    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            request.onsuccess = () => resolve((request.result as ContactI3C[]) || []);
...
export async function getSendingLog(): Promise<SendingLogEntry[]> {
    ...
    try {
        ...
        const request = store.getAll();
        return new Promise((resolve, reject) => {
            request.onsuccess = () => resolve((request.result as SendingLogEntry[]) || []);
...
// Gets all metadata key-value pairs from the METADATA store
    ...
    try {
        ...
        const [keys, values] = await Promise.all([
            ...
            new Promise<number[]>((resolve, reject) => {
                valuesRequest.onsuccess = () => resolve(valuesRequest.result as number[]);
...
export async function getLastImportExportDate(): Promise<number> {
    ...
    try {
        const transaction = db.transaction(STORE.METADATA, "readonly");
        const store = transaction.objectStore(STORE.METADATA);
        const request = store.get(METADATA_KEY.LAST_IMPORT_EXPORT_DATE);
        return new Promise((resolve, reject) => {
        ...
        console.error("Error retrieving last import/export date:", error);
        return 0;
    } finally {
        db.close();
    }
...
//     await removeItem("CONTACTS"); // Clears activeContacts store
//     await removeItem("LAST_IMPORT_EXPORT_DATE"); // Deletes specific metadata entry
// }
