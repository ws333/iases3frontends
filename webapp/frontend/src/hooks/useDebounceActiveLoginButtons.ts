...
export function useDebounceActiveLoginButtons({ isActiveGoogleLogin, isActiveMSLogin }: Props) {
  ...
  const bothActiveTimeRef = useRef<number | null>(null);
  ...
  // Google login check is the slowest so only measure the time and update rolling window if active goole login
  useEffect(() => {
    if (isActiveGoogleLogin.status) {
      const now = Date.now();
      const elapsed = Math.max(maxDebounceTime, now - mountTimeRef.current);
      bothActiveTimeRef.current = now;
