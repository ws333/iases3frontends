import { ContactI3CFax, ImportData, ImportStats } from '../types/typesI3C';

export interface ContactState {
  active: ContactI3CFax[]; // List of currently active contacts (dd = 0)
  deleted: ContactI3CFax[]; // List of deleted contacts (dd > 0)
...
export function mergeImportedContacts(
  ...
  const newLastImportExportDate = exportDate > current.lastImportExportDate ? exportDate : current.lastImportExportDate;
...
export function deleteContact(state: ContactState, uid: number, deletionTime: number): ContactState {
  ...
  const newDeleted: ContactI3CFax[] = [...state.deleted, { ...contact, dd: deletionTime }];
