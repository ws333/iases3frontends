...
export async function exportFromLocalStorage() {
  ...
  await storeMetadataKey(Date.now(), METADATA_KEY.EXPORT_DATE);
  ...
  for (const property in STORE) {
    ...
    if (data) {
      ...
      const dataToExport =
        key === 'activeContacts'
          ? JSON.stringify(
              (data as ContactI3CFax[]).map(
                (d): Partial<ContactI3CFax> => ({
                  uid: d.uid,
                  sd: d.sd,
                  sc: d.sc,
                  cf1: d.cf1,
  ...
  if (blob instanceof Blob) {
    ...
    a.href = url;
    a.download = 'iases3_fax_export.zip';
    a.click();
    URL.revokeObjectURL(url);
  } else {
    console.error('Export failed: zipWriter.close() did not return a Blob.');
  }
}
